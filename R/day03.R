...
#'
#' @param x some data
#' @return For Part One, `f03a(x)` returns the submarine power consumption. For
#'   Part Two, `f03b(x)` returns the life support metric.
#' @export
#' @examples
...
#' @rdname day03
#' @export
f03_make_matrix <- function(x) {
  purrr::map(strsplit(x, ""), ~t(as.matrix(as.numeric(.x)))) %>%
    purrr::reduce(rbind)
}

# Returns a function based on the rating type desired.
#' @rdname day03
#' @export
f03_apply_fun <- function(rate_type) {
  # Establish if else based on rate type
  if (rate_type == "gamma") {
    operator <- '>='
  } else if (rate_type == "epsilon") {
    operator <- '<'
  }
...
f03b <- function(x) {
  prod(
    purrr::map_dbl(c("gamma", "epsilon"), ~ {
      ...
      data <- f03_make_matrix(x) # Read in data
      col <- 1 # initial col value
      while (inherits(data, "matrix")) {
        com <- apply(data, 2, f03_apply_fun(.x))[col]
